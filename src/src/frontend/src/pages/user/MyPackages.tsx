    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          View all your purchased packages and payment status
        </p>
      </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center max-w-md">
              You haven't purchased any packages yet. Browse our pricing section
              to get started!
            </p>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold">
                        {new Date(
                          Number(proof.createdAt) / 1000000,
                        ).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                ...
                {proof.status === "pending" && (
                  <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    <p className="text-sm text-yellow-600 dark:text-yellow-500">
                      Your payment is being verified. This usually takes up to
                      24 hours.
                    </p>
                  </div>
                )}
                {proof.status === "approved" && (
                  <div className="mt-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                    <p className="text-sm text-green-600 dark:text-green-500">
                      Payment verified! You now have access to all courses in
                      this package.
                    </p>
                  </div>
                )}
                {proof.status === "rejected" && (
                  <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                    <p className="text-sm text-red-600 dark:text-red-500">
                      Payment verification failed. Please contact support for
                      assistance.
                    </p>
                  </div>
                )}
